...
path_tf_idf <- reactive({
    paste0("tf_idf_data/newspaper_lunion_", as.character(input$date2), "_tf_idf_data.rds")
})
...
path_textrank <- reactive({
    paste0("textrank_data/newspaper_lunion_", as.character(input$date2), "_textrank_data.rds")
})

dfInput2 <- reactive({
        readRDS(path_textrank())
})
